(function () {
    ...
    /**
     * Модифікація проєкту BazarNet | LampaUa.
     * Коментарі та службові позначки підтримуються українською мовою.
     */
    ...
    // ===================================================================
    // ВБУДОВАНА СИСТЕМА РЕЙТИНГІВ
    // ===================================================================

    /**
     * Конфігурація вбудованих рейтингів
     */
    ...
    /**
     * Кеш міток якості/озвучки для картки фільму.
     * Модифікація проєкту BazarNet | LampaUa:
     * зменшує повторні запити до парсера після перезапуску Lampa.
     */
    const QUALITY_BADGES_CACHE_CONFIG = {
        cacheLifetime: 12 * 60 * 60 * 1000, // 12 годин
        cacheKey: 'applecation_quality_badges_cache',
        cacheLimit: 1000,
        schemaVersion: 3
    };

    /**
     * Уніфіковані мережеві запити для рейтингів
     */
    ...
    /**
     * Утиліти для роботи із заголовками (для пошуку у КіноПошуку)
     */
    class TitleUtils {
        static clean(str) {
    ...
    /**
     * Менеджер кешу міток якості/озвучки для постера.
     */
    class QualityBadgesCacheManager {
        _getCache() {
            return Lampa.Storage.cache(
                QUALITY_BADGES_CACHE_CONFIG.cacheKey,
                QUALITY_BADGES_CACHE_CONFIG.cacheLimit,
                {}
            );
        }
        ...
        _makeKey(movie) {
            ...
            const parseLang = Lampa.Storage.field('parse_lang') || 'default';
            return [
                QUALITY_BADGES_CACHE_CONFIG.schemaVersion,
                mediaType,
                movie.id,
                parseLang
            ].join(':');
        ...
        get(movie) {
            ...
            const isExpired = (Date.now() - entry.timestamp) > QUALITY_BADGES_CACHE_CONFIG.cacheLifetime;
            const hasInvalidVersion = entry.version !== QUALITY_BADGES_CACHE_CONFIG.schemaVersion;
            const isLegacyEmptyEntry = entry.data
                && entry.data.torrents_found === 0
                && !entry.data.quality
                && !entry.data.dv
                && !entry.data.hdr
                && !entry.data.sound
            ...
            if (isExpired || hasInvalidVersion || isLegacyEmptyEntry || !entry.data || typeof entry.data !== 'object') {
                delete cache[key];
                Lampa.Storage.set(QUALITY_BADGES_CACHE_CONFIG.cacheKey, cache);
                return null;
            }
        ...
        set(movie, data) {
            ...
            const cache = this._getCache();
            cache[key] = {
                version: QUALITY_BADGES_CACHE_CONFIG.schemaVersion,
                timestamp: Date.now(),
                data: data
            };
            Lampa.Storage.set(QUALITY_BADGES_CACHE_CONFIG.cacheKey, cache);
    ...
    const qualityBadgesCacheManager = new QualityBadgesCacheManager();

    /**
     * Провайдер рейтингів MDBList
     */
        ...
        static _processResponse(response, callback) {
            ...
            if (response && response.ratings) {
                response.ratings.forEach(rating => {
                    ...
                    // Для Tomatoes/Popcorn нам потрібні додаткові поля (votes/url)
    ...
    /**
     * Провайдер рейтингів КіноПошуку
     */
    ...
    class BuiltInRatingsManager {
        ...
        fetch(movie, callback) {
            ...
            // Перевіряємо дублювання запросов
            ...
            // Якщо немає активних запросов (немає ключей), возвращаем порожній результат
            if (pendingRequests === 0) {
                result.ready = true;
    ...
    // Створюємо глобальний екземпляр менеджера рейтингів
    const builtInRatingsManager = new BuiltInRatingsManager();

    // ===================================================================
    // КІНЕЦЬ ВБУДОВАНОЇ СИСТЕМИ РЕЙТИНГІВ
    // ===================================================================

    /**
     * Проверяет, является ли активність все еще активной
     */
     ...
     * Витягує інформацію про роздільну здатність, HDR, Dolby Vision та аудіоканали
     */
    function analyzeContentQuality(ffprobe) {
        ...
        if (video) {
            ...
            if (video.width && video.height) {
                ...
                // Перевіряємо и ширину для широкоформатного контента (2.35:1, 2.39:1 и т.д.)
            ...
            // Альтернативная перевірка HDR через color_transfer
    ...
    /**
     * Створює порожній об'єкт міток якості для кешу/рендера.
     */
    function createEmptyQualityInfo(title, torrentsFound) {
        return {
            title: title || '',
            torrents_found: torrentsFound || 0,
            quality: null,
            dv: false,
            hdr: false,
            hdr_type: null,
    ...
    /**
     * Повертає об'єднаний текст з полів торрента для визначення міток.
     */
    function getTorrentSignalsText(torrent) {
    ...
    /**
     * Евристика по тексту торрента (fallback, коли ffprobe не дає всіх даних).
     */
    function collectQualitySignalsFromText(rawText, availableQualities) {
        if (!rawText || !availableQualities) return;

        const text = ` ${String(rawText).toLowerCase()} `.replace(/\s+/g, ' ');
        const hasToken = (pattern) => pattern.test(text);

        if (hasToken(/(^|[^a-z0-9])(4320p|8k)([^a-z0-9]|$)/)) {
            availableQualities.resolutions.add('8K');
        }
        if (hasToken(/(^|[^a-z0-9])(2160p|4k|uhd)([^a-z0-9]|$)/)) {
            availableQualities.resolutions.add('4K');
        }
        if (hasToken(/(^|[^a-z0-9])(1440p|2k)([^a-z0-9]|$)/)) {
            availableQualities.resolutions.add('2K');
        }
        if (hasToken(/(^|[^a-z0-9])(1080p|fhd|full\s*hd)([^a-z0-9]|$)/)) {
            availableQualities.resolutions.add('FULL HD');
        }
        if (hasToken(/(^|[^a-z0-9])(720p|hd)([^a-z0-9]|$)/)) {
            availableQualities.resolutions.add('HD');
        }

        if (hasToken(/(^|[^a-z0-9])7[ ._-]?1([^a-z0-9]|$)/)) {
            availableQualities.audio.add('7.1');
        } else if (hasToken(/(^|[^a-z0-9])5[ ._-]?1([^a-z0-9]|$)/)) {
            availableQualities.audio.add('5.1');
        } else if (hasToken(/(^|[^a-z0-9])4[ ._-]?0([^a-z0-9]|$)/)) {
            availableQualities.audio.add('4.0');
        } else if (hasToken(/(^|[^a-z0-9])2[ ._-]?0([^a-z0-9]|$)/)) {
            availableQualities.audio.add('2.0');
        }

        if (!availableQualities.hasUkr) {
            if (hasToken(/(^|[^a-z0-9])(ukr|ukrainian|ua|uk)([^a-z0-9]|$)/) || /укр|укра(ї|и)н/.test(text)) {
                availableQualities.hasUkr = true;
            }
        }

        if (!availableQualities.hasEng) {
            if (hasToken(/(^|[^a-z0-9])(eng|english|en)([^a-z0-9]|$)/)) {
                availableQualities.hasEng = true;
            }
        }

        if (!availableQualities.hasDub) {
            if (/дубляж|дубл|dublado|dubbed/.test(text) || hasToken(/(^|[^a-z0-9])dub([^a-z0-9]|$)/)) {
                availableQualities.hasDub = true;
            }
        }

        if (text.includes('dolby vision') || text.includes('dovi') || hasToken(/(^|[^a-z0-9])dv([^a-z0-9]|$)/)) {
    ...
    const likhtarQualityMemoryCache = {};
    const likhtarUafixMemoryCache = {};

    function getQualityRank(label) {
        const order = ['SD', 'HD', 'FULL HD', '2K', '4K', '8K'];
        return order.indexOf(label || 'SD');
    }

    function mapLikhtarResolutionToQuality(resolution) {
        if (!resolution) return null;
        if (resolution === 'FHD') return 'FULL HD';
        if (resolution === 'SD') return null;
        return resolution;
    }
    ...
    function fetchWithProxy(url, callback) {
        ...
        const tryProxy = (index) => {
            ...
            const requestUrl = proxy === 'https://api.allorigins.win/get?url='
            ...
            xhr.onerror = () => tryProxy(index + 1);
            xhr.ontimeout = () => tryProxy(index + 1);
            xhr.send();
        };
    ...
    function getLikhtarBestFromJacred(movie, callback) {
        if (!movie || !movie.id) {
            callback(null);
            return;
        }

        const cacheKey = 'applecation_likhtar_jacred_v1_' + movie.id;
        if (likhtarQualityMemoryCache[cacheKey]) {
            callback(likhtarQualityMemoryCache[cacheKey]);
            return;
        }

        try {
            const raw = Lampa.Storage.get(cacheKey, '');
            if (raw && typeof raw === 'object' && raw._ts && (Date.now() - raw._ts < 48 * 60 * 60 * 1000)) {
                likhtarQualityMemoryCache[cacheKey] = raw;
                callback(raw.empty ? null : raw);
                return;
            }
        } catch (e) { }

        const title = (movie.original_title || movie.original_name || movie.title || movie.name || '').toLowerCase();
        ...
        if (releaseDate && releaseDate.getTime && releaseDate.getTime() > Date.now()) {
            callback(null);
            return;
        }

        const apiUrl = 'https://jr.maxvol.pro/api/v1.0/torrents?search=' + encodeURIComponent(title) + '&year=' + year;
        fetchWithProxy(apiUrl, (err, data) => {
            if (err || !data) {
                callback(null);
                return;
            }
            ...
            try {
                ...
                const results = Array.isArray(parsed) ? parsed : (parsed && parsed.Results ? parsed.Results : []);
                if (!results.length) {
                    const emptyData = { empty: true, _ts: Date.now() };
                    likhtarQualityMemoryCache[cacheKey] = emptyData;
                    try { Lampa.Storage.set(cacheKey, emptyData); } catch (e) { }
                    callback(null);
                    return;
                }

                const bestGlobal = { resolution: 'SD', ukr: false, eng: false, hdr: false, dolbyVision: false };
                const bestUkr = { resolution: 'SD', ukr: false, eng: false, hdr: false, dolbyVision: false };
                const resOrder = ['SD', 'HD', 'FHD', '2K', '4K'];

                results.forEach((item) => {
                    const t = String(item.title || item.Title || '').toLowerCase();
                    ...
                    if (t.indexOf('dolby vision') >= 0 || t.indexOf('dolbyvision') >= 0 || t.indexOf('dovi') >= 0) {
                        isHdr = true;
                        isDv = true;
                ...
                const finalBest = bestGlobal.ukr ? bestUkr : bestGlobal;
                if (movie.original_language === 'en') finalBest.eng = true;
                finalBest.empty = false;
                finalBest._ts = Date.now();

                likhtarQualityMemoryCache[cacheKey] = finalBest;
    ...
    function checkLikhtarUafixBandera(movie, callback) {
        const title = movie.title || movie.name || '';
        const originalTitle = movie.original_title || movie.original_name || '';
        const imdbId = movie.imdb_id || '';
        const type = movie.name ? 'series' : 'movie';

        let url = 'https://banderabackend.lampame.v6.rocks/api/v2/search?source=uaflix';
        if (title) url += '&title=' + encodeURIComponent(title);
        if (originalTitle) url += '&original_title=' + encodeURIComponent(originalTitle);
    ...
    function checkLikhtarUafixDirect(movie, callback) {
        const query = movie.original_title || movie.original_name || movie.title || movie.name || '';
        if (!query) {
            callback(false);
            return;
        }
        ...
        fetchWithProxy(searchUrl, (err, html) => {
            if (err || !html) {
                callback(false);
                return;
            }
            const hasResults = html.indexOf('знайдено') >= 0 && html.indexOf('0 відповідей') < 0;
            callback(hasResults);
        });
    }

    function checkLikhtarUafix(movie, callback) {
        if (!movie || !movie.id) {
            callback(false);
            return;
        }

        const key = 'applecation_uafix_' + movie.id;
        if (likhtarUafixMemoryCache[key] !== undefined) {
            callback(likhtarUafixMemoryCache[key]);
            return;
        }

        checkLikhtarUafixBandera(movie, (result) => {
            if (result !== null) {
                likhtarUafixMemoryCache[key] = result;
                callback(result);
                return;
            }

            checkLikhtarUafixDirect(movie, (found) => {
                likhtarUafixMemoryCache[key] = found;
                callback(found);
            });
        });
    }

    function mergeLikhtarQualityIntoInfo(baseQualityInfo, likhtarData) {
        const merged = Object.assign({}, baseQualityInfo || {});
        if (!likhtarData || likhtarData.empty) return merged;

        const mappedResolution = mapLikhtarResolutionToQuality(likhtarData.resolution);
        if (mappedResolution) {
            const currentRank = getQualityRank(merged.quality || 'SD');
            const incomingRank = getQualityRank(mappedResolution);
            if (incomingRank > currentRank) {
                merged.quality = mappedResolution;
            }
        }

        if (likhtarData.ukr) merged.ukr = true;
        if (likhtarData.eng) merged.eng = true;
        if (likhtarData.hdr) merged.hdr = true;
        if (likhtarData.dolbyVision) {
            merged.dv = true;
            merged.hdr = true;
        }
        if (likhtarData.hdr && !likhtarData.dolbyVision && !merged.hdr_type) {
            merged.hdr_type = 'HDR';
        }
    ...
    function enrichQualityInfoFromLikhtar(movie, baseQualityInfo, callback) {
        if (!movie || !movie.id) {
            callback(baseQualityInfo);
            return;
        }

        getLikhtarBestFromJacred(movie, (data) => {
            let bestData = data || { empty: true };

            const finish = () => {
                callback(mergeLikhtarQualityIntoInfo(baseQualityInfo, bestData));
            };
            ...
            checkLikhtarUafix(movie, (hasUafix) => {
                if (hasUafix) {
                    if (bestData.empty) bestData = { empty: false, resolution: 'FHD', hdr: false, dolbyVision: false, ukr: false, eng: false };
                    bestData.ukr = true;
    ...
    function analyzeContentQualities(movie, activity) {
        ...
        if (activity) activity.applecation_movie = movie;
        const title = movie.title || movie.name || 'Unknown';

        const applyQualityInfo = (qualityInfo) => {
            const safeQualityInfo = qualityInfo && typeof qualityInfo === 'object'
                ? qualityInfo
                : createEmptyQualityInfo(title, 0);

            if (!safeQualityInfo.ukr && (movie.original_language === 'uk' || movie.original_language === 'ua')) {
                safeQualityInfo.ukr = true;
            }

            const hasAnyMarks = !!(
                safeQualityInfo.quality ||
                safeQualityInfo.dv ||
                safeQualityInfo.hdr ||
                safeQualityInfo.hdr_type ||
                safeQualityInfo.sound ||
                safeQualityInfo.dub ||
                safeQualityInfo.ukr ||
                safeQualityInfo.eng
            );
            if (hasAnyMarks) {
                qualityBadgesCacheManager.set(movie, safeQualityInfo);
            }

            if (activity && isAlive(activity)) {
                activity.applecation_quality = safeQualityInfo;
                updateQualityBadges(activity, safeQualityInfo);
                refreshInlineMetaStrip(activity, movie, safeQualityInfo);
            }
        };

        const enrichAndApply = (baseQualityInfo) => {
            applyQualityInfo(baseQualityInfo);
            enrichQualityInfoFromLikhtar(movie, baseQualityInfo, (enrichedQualityInfo) => {
                if (activity && !isAlive(activity)) return;
                applyQualityInfo(enrichedQualityInfo || baseQualityInfo);
            });
        };

        const cachedQualityInfo = qualityBadgesCacheManager.get(movie);
        if (cachedQualityInfo) {
            applyQualityInfo(cachedQualityInfo);
            if (!cachedQualityInfo.ukr) {
                enrichAndApply(cachedQualityInfo);
            }
            return;
        }

        const parserEnabled = !!Lampa.Storage.field('parser_use');
        const parserAvailable = !!(Lampa.Parser && typeof Lampa.Parser.get === 'function');

        if (!parserEnabled || !parserAvailable) {
            enrichAndApply(createEmptyQualityInfo(title, 0));
            return;
        }
        ...
        Lampa.Parser.get({
            ...
            results.Results.forEach((torrent) => {
                const signalsText = getTorrentSignalsText(torrent);
                collectQualitySignalsFromText(signalsText, availableQualities);
                ...
                if (torrent.ffprobe && Array.isArray(torrent.ffprobe)) {
                    ...
                    audioStreams.forEach(audio => {
                        ...
                        const streamTitle = (audio.tags.title || audio.tags.handler_name || '').toLowerCase();

                        if (!availableQualities.hasUkr && (lang === 'uk' || lang === 'ukr' || lang === 'ukrainian')) {
                            availableQualities.hasUkr = true;
                        }
                        if (!availableQualities.hasEng && (lang === 'en' || lang === 'eng' || lang === 'english')) {
                            availableQualities.hasEng = true;
                        }

                        if (!availableQualities.hasDub && (lang === 'rus' || lang === 'ru' || lang === 'russian')) {
                            if (streamTitle.includes('dub') || streamTitle.includes('дубляж') ||
                                streamTitle.includes('дублир') || streamTitle === 'd') {
                                availableQualities.hasDub = true;
                            }
                        }
                    });
            ...
            const qualityInfo = createEmptyQualityInfo(title, results.Results.length);
            qualityInfo.dub = availableQualities.hasDub;
            qualityInfo.ukr = availableQualities.hasUkr;
            qualityInfo.eng = availableQualities.hasEng;
    ...
    /**
     * Патч логіки лінії епізодів (як окремий episodes_core_patch.js, але інтегровано в Applecation)
     * Цель:
     * - епізоди завжди йдуть 1..N, потім next (comeing)
     * - кнопка "Ще" (card-more) завжди остання при лінивому догрузі
    ...
    function attachEpisodesCorePatch(){
        try{
            ...
            // якщо десь отдельно подключен старий плагін-перестановщик — вимикаємо его флагом
            ...
            function patchScrollAppendToKeepMoreLast(line){
                try{
                    ...
                    line.scroll.append = function(object){
                        ...
                        // Якщо "Ще" уже есть — вставляем перед ним
            ...
            function patchLineCreate(line){
                try{
                    ...
                    line.create = function(){
                        ...
                        // прибираємо стили "первого" у "Ще" (якщо MoreFirst навесил)
                        setTimeout(function(){
                            try{
    ...
    const translations = {
        ...
        quality_badges_cache_clear: {
            ru: 'Очистить кеш меток качества',
            en: 'Clear quality badges cache',
            uk: 'Очистити кеш міток якості',
            be: 'Ачысціць кеш пазнак якасці',
            bg: 'Изчисти кеша на етикетите за качество',
            cs: 'Vymazat cache štítků kvality',
            he: 'נקה מטמון תגי איכות',
            pt: 'Limpar cache de etiquetas de qualidade',
            zh: '清除画质标签缓存'
        },
        quality_badges_cache_clear_desc: {
            ru: 'Сбрасывает кеш HDR/DV/звук/дубляж для повторного анализа',
            en: 'Resets HDR/DV/sound/dub cache for re-analysis',
            uk: 'Скидає кеш HDR/DV/звук/дубляж для повторного аналізу',
            be: 'Скідвае кеш HDR/DV/гук/дубляж для паўторнага аналізу',
            bg: 'Нулира кеша за HDR/DV/звук/дублаж за повторен анализ',
            cs: 'Resetuje cache HDR/DV/zvuk/dabing pro opětovnou analýzu',
            he: 'מאפס מטמון HDR/DV/שמע/דיבוב לניתוח מחדש',
            pt: 'Redefine o cache de HDR/DV/áudio/dublagem para nova análise',
            zh: '重置 HDR/DV/音频/配音缓存以重新分析'
        },
        quality_badges_cache_cleared: {
            ru: 'Кеш меток качества очищен',
            en: 'Quality badges cache cleared',
            uk: 'Кеш міток якості очищено',
            be: 'Кеш пазнак якасці ачышчаны',
            bg: 'Кешът на етикетите за качество е изчистен',
            cs: 'Cache štítků kvality byla vymazána',
            he: 'מטמון תגי האיכות נוקה',
            pt: 'Cache de etiquetas de qualidade limpo',
            zh: '画质标签缓存已清除'
        },
        about_author: {
            ru: 'Модификация проекта',
            en: 'Project modification',
            uk: 'Модифікація проєкту',
            be: 'Мадыфікацыя праекта',
            bg: 'Модификация на проекта',
            cs: 'Úprava projektu',
            he: 'מודיפיקציית פרויקט',
            pt: 'Modificação do projeto',
            zh: '项目修改'
        },
        about_description: {
            ru: 'Модификация проекта BazarNet | LampaUa: интерфейс карточки в стиле Apple TV и оптимизация под 4K',
            en: 'BazarNet | LampaUa project modification: Apple TV-style movie card interface with 4K optimization',
            uk: 'Модифікація проєкту BazarNet | LampaUa: інтерфейс картки у стилі Apple TV та оптимізація під 4K',
            be: 'Мадыфікацыя праекта BazarNet | LampaUa: інтэрфейс карткі ў стылі Apple TV і аптымізацыя пад 4K',
            bg: 'Модификация на проекта BazarNet | LampaUa: интерфейс на картата в стил Apple TV и оптимизация за 4K',
            cs: 'Úprava projektu BazarNet | LampaUa: rozhraní filmové karty ve stylu Apple TV a optimalizace pro 4K',
            he: 'מודיפיקציית פרויקט BazarNet | LampaUa: ממשק כרטיס סרט בסגנון Apple TV עם אופטימיזציה ל-4K',
            pt: 'Modificação do projeto BazarNet | LampaUa: interface do cartão de filme no estilo Apple TV com otimização para 4K',
            zh: 'BazarNet | LampaUa 项目修改：Apple TV 风格电影卡片界面并优化 4K'
        }
    };
    ...
    // Застосовуємо клас для керування ефектом рідкого скла
    ...
    function addSettings() {
        ...
        if (Lampa.Storage.get('applecation_enabled_ratings') === undefined) {
            // По умолчанию не включаем КП, якщо ключ не задан
            ...
            // Якщо ключа КП немає — прибираємо КП зі списку відображуваних рейтингів
            const hasKpKey = !!Lampa.Storage.get('applecation_kp_api_key', '');
            if (!hasKpKey) {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: '<div>Applecation v' + APPLECATION_VERSION + ' • BazarNet | LampaUa</div>'
            },
            onRender: function(item) {
                ...
                item.append('<div style="font-size: 0.9em; padding: 0 1.2em; line-height: 1.4;"><b>BazarNet | LampaUa</b><br>' + t('about_description') + '</div>');
            }
        });
        ...
        // Службова дія: очистити кеш міток якості/озвучки (HDR/DV/звук/дубляж).
        Lampa.SettingsApi.addParam({
            component: 'applecation_settings',
            param: {
                name: 'applecation_quality_badges_cache_clear',
                type: 'button'
            },
            field: {
                name: t('quality_badges_cache_clear'),
                description: t('quality_badges_cache_clear_desc')
            },
            onChange: function() {
                Lampa.Storage.set(QUALITY_BADGES_CACHE_CONFIG.cacheKey, {});
                Lampa.Noty.show(t('quality_badges_cache_cleared'));
            }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                ...
                // Оновлюємо видимість залежних параметров
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                ...
                // Перемикаємо CSS-класи для приховування/показу потрібних рейтингів
        ...
        // MDBList API Key (лише для вбудованих рейтингів)
        Lampa.SettingsApi.addParam({
            component: 'applecation_settings',
            param: {
        ...
        // КиноПоиск API Key (лише для вбудованих рейтингів)
        Lampa.SettingsApi.addParam({
            component: 'applecation_settings',
            param: {
            ...
            onChange: function() {
                ...
                Lampa.Input.edit({
                    ...
                    if (newValue !== currentKey) {
                        ...
                        // Якщо ключ видалили — прибираємо КП зі списку відображуваних рейтингів
                        if (!newValue) {
        ...
        // Рейтинги що відображаються (лише для вбудованих рейтингів)
        Lampa.SettingsApi.addParam({
            component: 'applecation_settings',
            param: {
            ...
            onChange: function() {
                ...
                // Якщо ключа КП немає — не даємо вибрати та очищаємо зі збереженого списку
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                ...
                // Оновлюємо шаблони и перезавантажуємо активність
                addCustomTemplate();
                addOverlayTemplate();
                Lampa.Activity.back();
            },
        ...
        // Показувати реакції (дефолтная настройка Lampa)
        Lampa.SettingsApi.addParam({
            component: 'applecation_settings',
            param: {
        ...
        // Застосовуємо текущие настройки
        ...
        // Застосовуємо клас для джерелоа рейтингів
        const ratingsSource = Lampa.Storage.get('applecation_ratings_source', 'external');
    ...
    // Застосовуємо масштабирование контента
    function applyScales() {
        ...
        // Видаляємо старі стили якщо есть
        $('style[data-id="applecation_scales"]').remove();

        // Створюємо нові стили
        const scaleStyles = `
    ...
    function disableFullDescription(e) {
        if (e.type === 'start' && e.link) {
            // Видаляємо 'description' из списку rows перед рендерингом
...
/* Вбудовані рейтинги: плавна поява контента всередині вже показуного блоку */
...
/* Кожен рейтинг анімується під час вставки. До розкриття картки анімація на паузі. */
...
/* Управление видимістью рейтингів через настройки */
body.applecation--hide-ratings .applecation__ratings {
    display: none !important;
}

/* Приховування рейтингів залежно від джерела */
/* Коли обрані вбудовані рейтинги - приховуємо контейнери для зовнішніх плагінів */
...
/* Коли обрані зовнішні рейтинги - приховуємо вбудовані */
body.applecation--ratings-source-external .applecation__ratings-builtin {
    display: none !important;
}

/* Розташування рейтингів - в правом нижнем куті */
...
/* Дополнительная информация (Год/тривалість) */
.applecation__info {
...
.applecation__merge-strip.show {
    opacity: 1;
    transform: translateY(0);
}

.applecation-inline-badge {
    display: inline-flex;
    align-items: center;
    gap: 0.28em;
    padding: 0.2em 0.5em;
...
.applecation-inline-badge--rating {
    background: rgba(20, 32, 62, 0.72);
    border-color: rgba(110, 149, 255, 0.5);
}

.applecation-inline-badge__value {
    font-weight: 700;
}

.applecation-inline-badge__label {
    font-size: 0.86em;
    opacity: 0.8;
    text-transform: uppercase;
}

.applecation-inline-badge--ua {
    background: rgba(35, 133, 232, 0.78);
}

.applecation-inline-badge--en {
    background: rgba(91, 110, 130, 0.78);
}

.applecation-inline-badge--quality {
    background: rgba(132, 43, 190, 0.8);
}

.applecation-inline-badge--dv,
.applecation-inline-badge--hdr {
    background: rgba(42, 48, 60, 0.8);
}

.applecation--inline-meta-has-quality .applecation__quality-badges {
    display: none !important;
}

.applecation--inline-meta-has-ratings .applecation__ratings {
    display: none !important;
}
...
/* Вирівнювання по baseline якщо рейтинги у куті */
body.applecation--ratings-corner .applecation__right {
...
/* Стили первой реакции (завжди видимой) */
...
/* При фокусі реакции розкриваються вгору */
...
/* Приховуємо стандартний rate-line (використовується лише для статусу) */
...
/* Приховуємо статус для запобігання виходу реакций за екран */
...
/* без фокуса — взагалі без ефектів */
...
/* когда фокуса немає — просто не показуємо слои стекла */
...
/* Зміщуємо обличчя лише при високій якості (w500), оскільки там інший кроп у TMDB */
.applecation.applecation--poster-high .full-person__photo img {
    object-position: center calc(50% + 20px) !important;
}

/* Типові заглушки лишаємо по центру, щоб не ламати симетрію иконок */
...
/* При фокусі (когда рядок едет) прозрачность с обеих сторон */
...
body.applecation--no-liquid-glass .applecation .full-person.focus .full-person__photo::after {
    ...
    // Патчимоо внутрішні методи Lampa для коректної роботи епізодов и качества
    function patchApiImg() {
        ...
        if (window.Lampa && Lampa.TMDB && Lampa.TMDB.api) {
            ...
            Lampa.TMDB.api = function(url) {
                ...
                if (typeof newUrl === 'string' && newUrl.indexOf('append_to_response=') !== -1 && newUrl.indexOf('images') === -1) {
                    // Додаємо images в список append_to_response
                    newUrl = newUrl.replace('append_to_response=', 'append_to_response=images,');
                    
                    // Додаємо мовуи для картинок, якщо вони ще не вказані
        ...
        Lampa.Template.js = function(name, vars) {
            if (name === 'full_episode' && vars) {
                // Форматируем время (локализовано: 1 ч 10 м или 39 м) - прибираємо точки
                if (vars.runtime > 0) {
                ...
                // Форматируем дату: завжди с годом
                if (vars.air_date) {
        ...
        // 2. Патчимо метод изображений для улучшения качества
    ...
    // Отримуємо качество логотипа на основе poster_size
    ...
    function loadNetworkIcon(activity, data) {
        ...
        // Якщо немає иконки - приховуємо контейнер
        networkContainer.remove();
    }

    // Заполняем мета інформацію (Тип/Жанр/поджанр)
    ...
    function fillDescription(activity, data) {
        ...
        if (useOverlay) {
            // Створюємо оверлей заранее
            createDescriptionOverlay(activity, data);
            
            // Додаємо обработчик клика для показу полного описания
            descWrapper.off('hover:enter').on('hover:enter', function() {
                showFullDescription();
            });
        } else {
            // Якщо оверлей отключен, прибираємо обработчики и видаляємо оверлей
            descWrapper.off('hover:enter');
            $('.applecation-description-overlay').remove();
        }
    }
    
    // Оновлюємо логотип в оверлее
    ...
    // Парсимоо країни с локалізацією (як у ядрі Lampa)
    function parseCountries(movie) {
        ...
        return movie.production_countries.map(country => {
            ...
            // Якщо перевод найден (не равен ключу), используем его, иначе оригинальное имя
    ...
    function createDescriptionOverlay(activity, data) {
        ...
        // Видаляємо старий оверлей якщо есть
        $('.applecation-description-overlay').remove();
        
        // Парсимоо дані как в Lampa
        ...
        // Приховуємо країни якщо порожньо
        if (!countries) {
            overlay.find('.applecation--countries').remove();
        }
        
        // Додаємо в body но НЕ показуємо
        $('body').append(overlay);
    ...
    // Показуємо полное описание в оверлее
    function showFullDescription() {
        ...
        // Створюємо контроллер лише один раз
    ...
    // Закриваємо оверлей с описанием
    ...
    // Склонение сезонов с локалізацією
    function formatSeasons(count) {
        const lang = Lampa.Storage.get('language', 'ru');
        
        // Слов'янські мовуи (ru, uk, be, bg) - складне відмінювання
    ...
    // Заполняем дополнительную інформацію (Год/тривалість)
    function fillAdditionalInfo(activity, data) {
        ...
        // Длительность
        if (data.name) {
            // Сериал - показуємо и продолжительность епізода, и количество сезонов
            ...
            // Всегда показуємо количество сезонов для сериалов
            const seasons = Lampa.Utils.countSeasons(data);
            if (seasons) {
                infoParts.push(formatSeasons(seasons));
            }

            // Показуємо количество серий, якщо включено в настройках
        ...
        infoContainer.html(textContent + '<span class="applecation__quality-badges"></span><div class="applecation__merge-strip"></div>');
    }

    function mapInlineQualityLabel(value) {
        if (!value) return '';
        if (value === 'FULL HD') return '1080p';
        if (value === 'HD') return '720p';
        return value;
    }

    function getRatingLabelByKey(key) {
    ...
    function collectInlineRatings(activity, movie) {
        const result = [];
        const showRatings = Lampa.Storage.get('applecation_show_ratings', false);
        if (!showRatings) return result;

        const pushed = {};
        const pushRating = function (key, value) {
            if (pushed[key]) return;
            const num = parseRatingValue(value);
            if (isNaN(num) || num <= 0) return;
            pushed[key] = true;
            result.push({ key: key, label: getRatingLabelByKey(key), value: num.toFixed(1) });
        };
        ...
        const builtInRatings = activity && activity.applecation_ratings ? activity.applecation_ratings : null;
        if (builtInRatings && typeof builtInRatings === 'object') {
            ['tmdb', 'imdb', 'kp'].forEach((key) => {
                if (enabledRatings.indexOf(key) === -1) return;
                const raw = builtInRatings[key];
                const rawValue = (raw && typeof raw === 'object') ? (raw.score ?? raw.value) : raw;
                pushRating(key, rawValue);
            });
        }

        const renderEl = activity && activity.render ? activity.render() : null;
        if (renderEl && renderEl.length) {
            const domSelectors = {
                tmdb: '.rate--tmdb div, .builtin-rate--tmdb div',
                imdb: '.rate--imdb div, .builtin-rate--imdb div',
                kp: '.rate--kp div, .builtin-rate--kp div'
            };
            ['tmdb', 'imdb', 'kp'].forEach((key) => {
                if (enabledRatings.indexOf(key) === -1) return;
                const valText = renderEl.find(domSelectors[key]).first().text().trim();
                if (valText) pushRating(key, valText);
            });
        }

        if (movie && movie.vote_average) {
            pushRating('tmdb', movie.vote_average);
        }

        return result;
    }

    function collectInlineQuality(qualityInfo) {
        const parts = [];
        if (!qualityInfo || typeof qualityInfo !== 'object') return parts;

        const res = mapInlineQualityLabel(qualityInfo.quality);
        if (res) parts.push({ type: 'quality', label: res });

        if (qualityInfo.dv) parts.push({ type: 'dv', label: 'DV' });
        else if (qualityInfo.hdr && qualityInfo.hdr_type) parts.push({ type: 'hdr', label: qualityInfo.hdr_type });
        else if (qualityInfo.hdr) parts.push({ type: 'hdr', label: 'HDR' });

        if (qualityInfo.ukr) parts.push({ type: 'ua', label: 'UA' });
        if (qualityInfo.eng) parts.push({ type: 'en', label: 'EN' });

        return parts;
    }

    function renderInlineMetaStrip(activity, movie, qualityInfo) {
        if (!activity || !isAlive(activity)) return;

        const root = activity.render();
        const strip = root.find('.applecation__merge-strip');
        if (!strip.length) return;

        const ratingParts = collectInlineRatings(activity, movie);
        const qualityParts = collectInlineQuality(qualityInfo);
        const primaryRating = ratingParts.find(r => r.key === 'tmdb') || ratingParts[0] || null;
        const secondaryRatings = primaryRating
            ? ratingParts.filter(r => r !== primaryRating)
            : [];
        const htmlParts = [];

        if (primaryRating) {
            htmlParts.push(
                `<span class="applecation-inline-badge applecation-inline-badge--rating"><span class="applecation-inline-badge__value">${primaryRating.value}</span><span class="applecation-inline-badge__label">${primaryRating.label}</span></span>`
            );
        }

        secondaryRatings.forEach((r) => {
            htmlParts.push(
                `<span class="applecation-inline-badge applecation-inline-badge--rating"><span class="applecation-inline-badge__value">${r.value}</span><span class="applecation-inline-badge__label">${r.label}</span></span>`
            );
        });

        qualityParts.forEach((q) => {
            htmlParts.push(`<span class="applecation-inline-badge applecation-inline-badge--${q.type}">${q.label}</span>`);
        });

        if (!htmlParts.length) {
            strip.removeClass('show').empty();
            root.removeClass('applecation--inline-meta-has-ratings applecation--inline-meta-has-quality');
            return;
        }

        strip.html(htmlParts.join('')).addClass('show');

        if (ratingParts.length) root.addClass('applecation--inline-meta-has-ratings');
        else root.removeClass('applecation--inline-meta-has-ratings');

        if (qualityParts.length) root.addClass('applecation--inline-meta-has-quality');
        else root.removeClass('applecation--inline-meta-has-quality');
    }

    function refreshInlineMetaStrip(activity, movie, qualityInfo) {
        if (!activity || !isAlive(activity)) return;
        const resolvedMovie = movie || activity.applecation_movie || null;
        const resolvedQuality = qualityInfo || activity.applecation_quality || null;
        renderInlineMetaStrip(activity, resolvedMovie, resolvedQuality);
    }
    
    // Оновлюємо бейджи качества
    function updateQualityBadges(activity, qualityInfo) {
        ...
        refreshInlineMetaStrip(activity, activity.applecation_movie, qualityInfo);
    }

    // Загрузка и отображение рейтингів
    function loadAndDisplayRatings(activity, movie) {
        if (movie) activity.applecation_movie = movie;

        // Перевіряємо, чи увімкнені рейтинги
        if (!Lampa.Storage.get('applecation_show_ratings', false)) {
            refreshInlineMetaStrip(activity, movie, activity.applecation_quality);
            return;
        }
        ...
        if (ratingsSource === 'builtin') {
            ...
            // Зовнішні плагіни (наявний функціонал)
            // Рейтинги будутьь додані плагінами через DOM
    ...
    // Отображение рейтингів в интерфейсе
    function displayRatings(activity, ratings, movie) {
        ...
        activity.applecation_ratings = ratings && typeof ratings === 'object' ? ratings : {};
        ...
        // Скидаємо попередній контент та анімацію
        builtInContainer.removeClass('show').addClass('hide').empty();

        // Используем префикс класса в зависимости от джерелоа
        const classPrefix = 'builtin-rate--';

        // Порядок отображения всех рейтингів
        ...
        // Показуємо контейнер якщо есть рейтинги
    ...
    function loadLogo(event) {
        ...
        activity.applecation_movie = data;

        // Заполняем основную інформацію
        fillMetaInfo(activity, data);
        fillDescription(activity, data);
        fillAdditionalInfo(activity, data);
        refreshInlineMetaStrip(activity, data, activity.applecation_quality || createEmptyQualityInfo(data.title || data.name, 0));

        // Ждем когда фон загрузится и появится
        waitForBackgroundLoad(activity, () => {
            if (!isAlive(activity)) return;

            // После загрузки фона показуємо контент
        ...
        if (data.images && data.images.logos && data.images.logos.length > 0) {
            // Знаходимо логотип на поточній мовуе или англійська/нейтральна
            const lang = Lampa.Storage.field('tmdb_lang') || Lampa.Storage.get('language') || 'ru';
            let logo = data.images.logos.find(l => l.iso_639_1 === lang);
            
            // Якщо логотипа на поточній мовуе немає, шукаємо на английском или нейтральном
        ...
        // 2. Якщо логотипа немає в даних (наприклад, інше джерело або помилка патча), робимо старий запит
        ...
        $.get(apiUrl, (imagesData) => {
            ...
            // Нет подходящего логотипа - показуємо текстовое название
            titleElement.show();
            waitForBackgroundLoad(activity, () => {
                logoContainer.addClass('loaded');
            });
        }).fail(() => {
            // При ошибке показуємо текстовое название
            titleElement.show();
            waitForBackgroundLoad(activity, () => {
                logoContainer.addClass('loaded');
            });
    ...
    function waitForBackgroundLoad(activity, callback) {
        ...
        // Якщо фон уже загружен и анимация завершена
        if (background.hasClass('loaded') && background.hasClass('applecation-animated')) {
            callback();
            return;
        }

        // Якщо фон загружен но анимация еще идет
        if (background.hasClass('loaded')) {
            // Ждем завершения transition + небольшая затримка для надежности
            setTimeout(() => {
        ...
        const checkInterval = setInterval(() => {
            ...
            if (background.hasClass('loaded')) {
                clearInterval(checkInterval);
                // Ждем завершения transition + небольшая затримка
                setTimeout(() => {
                    if (!isAlive(activity)) return;
        ...
        // Таймаут на случай якщо что-то пошло не так
        setTimeout(() => {
            clearInterval(checkInterval);
    ...
    // Застосовуємо розмиття фону під час прокрутки
    function attachScrollBlur(activity) {
        ...
        // Перехоплюємо сетер стилю - найшвидший і синхронний спосіб
        ...
        Object.defineProperty(scrollBody.style, '-webkit-transform', {
            set: function(value) {
                // Оптимізований парсинг без regex
                if (value) {
                    ...
                    if (yStart > 0 && yEnd > yStart) {
                        ...
                        // Застосовуємо лише якщо состояние изменилось
                ...
                // Викликаємо оригінальний сеттер
    ...
    function attachPersonMarquee(activity) {
        ...
        function isTextOverflowing(element) {
            // Для коректної перевірки на прихованих елементах или в процессе отрисовки
            return element.scrollWidth > element.clientWidth + 1;
        }
        
        // Инициализируем marquee для тех, кто переполнен
        // Невелика затримка, щоб layout встиг перерахуватись
        setTimeout(() => {
            if (!isAlive(activity)) return;
            ...
            names.each(function() {
                ...
                if (isTextOverflowing(nameElement[0])) {
                    // Розраховуємо тривалість: ~250мс на символ, но не менее 5с и не более 20с
    ...
    function attachLogoLoader() {
        Lampa.Listener.follow('full', (event) => {
            // Отключаем блок "Подробно", якщо включен оверлей
            ...
            if (event.type === 'complite') {
                ...
                // Додаємо класс для применения стилей
                render.addClass('applecation');

                // Помечаем активність при уничтожении
                activity.__destroyed = false;
                
                // Зберігаємо оригінальний метод destroy якщо он есть
                ...
                // Додаємо класс качества постеров для CSS
                const posterSize = Lampa.Storage.field('poster_size');
    ...
    // Регистрация плагіна в манифесте
    var pluginManifest = {
        type: 'other',
        version: APPLECATION_VERSION,
        name: 'Applecation',
        description: 'BazarNet | LampaUa customization of Applecation UI',
        author: 'BazarNet | LampaUa',
        icon: PLUGIN_ICON
    };
