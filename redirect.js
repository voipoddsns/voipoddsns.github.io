...
Lampa.Lang.add({
	...
	server_select_desc: {
		ru: 'Выберите один из серверов',
		uk: 'Оберіть один із серверів',
		en: 'Select one of the servers'
	},
	server_redirect_const: {
		ru: 'Постоянный редирект',
		uk: 'Постійний редирект',
		en: 'Permanent redirect'
	},
	server_redirect_const_desc: {
		ru: 'Зажмите ВНИЗ при запуске чтобы отключить',
		uk: 'Затисніть ВНИЗ при запуску щоб вимкнути',
		en: 'Hold DOWN during startup to disable'
	}
});
...
function startMe() {
	...
	$('#REDIRECT').on('hover:enter hover:click hover:touch', function() {
		var index = Lampa.Storage.get('location_server_index') || 0;
		var domain = servers[index] || servers[0];

		window.location.href = server_protocol + domain;
	});
	...
	Lampa.SettingsApi.addComponent({
		...
		name: Lampa.Lang.translate('server_redirect'),
	...
	Lampa.SettingsApi.addParam({
		component: 'location_redirect',
		param: {
			name: 'location_server_index',
			type: 'select',
			values: servers,
			default: 0
		},
		field: {
			name: Lampa.Lang.translate('server_select'),
			description: Lampa.Lang.translate('server_select_desc')
		},
		onChange: function(value) {

			Lampa.Storage.set('location_server_index', value);
			...
			setTimeout(function() {
				...
				if (typeof value === 'number' || !isNaN(value)) {
					domain = servers[parseInt(value)] || servers[0];
				}
				else {
					domain = value;
				}
	...
	Lampa.SettingsApi.addParam({
		...
		field: {
			name: Lampa.Lang.translate('server_redirect_const'),
			description: Lampa.Lang.translate('server_redirect_const_desc')
		}
	});
